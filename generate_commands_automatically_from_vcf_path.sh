...
echo -e " #################################### "
echo 
echo -e "Usage:    bash script.sh VCF_PATH OUT_ROOT START STOP (CONFIRM) > LOG"
echo
...
echo
echo -e "Example:  bash generate_commands_from_vcf_path.sh /mnt/stsi/stsi0/raqueld/vcf/SHARE_MESA_c2_flipfix.vcf /mnt/stsi/stsi0/raqueld 0 6 > MESA_jobs_c1.txt"
echo 

# Default settings
if [ -z $2 ]; then exit; else myinput=$1; fi
if [ -z $2 ]; then outroot="/mnt/stsi/stsi0/raqueld"; else outroot=$2; fi
if [ -z $3 ]; then start_from=999; else start_from=$3; fi
if [ -z $4 ]; then stop_after=999; else stop_after=$4; fi
if [ -z $5 ]; then run=0; else run=1; fi

echo "User input: VCF_PATH: $1"
echo "            OUT_ROOT: $2"
echo "User option: START=${start_from}, STOP=${stop_after}, RUN=${run}"
echo 
...
job2='qsub 2_Genotype_Harmonizer_QC1.job -v myinput=${outroot}/1_lift/${prefix}.lifted_NCBI36_to_GRCh37.bed,myoutdir=${outroot}/2_GH -N 2_${prefix}'
job3='qsub 3_ancestry_analysis.job -v myinput=${outroot}/2_GH/${prefix}.lifted_NCBI36_to_GRCh37.GH.vcf.gz,myoutdir=${outroot}/3_ancestry -N 3_${prefix}'
job4='qsub 4_split_QC2.job -v myinput=${outroot}/3_ancestry/${prefix}/${prefix}.lifted_NCBI36_to_GRCh37.GH.ancestry-${anc}.bed,myoutdir=${outroot}/4_split_QC2,geno=0.1,mind=0.05 -N 4_${prefix}'
job5='qsub 5_phase.job -v myinput=${outroot}/4_split_QC2/${prefix}/${prefix}.lifted_NCBI36_to_GRCh37.GH.ancestry-${anc}.chr${chrom}.bed,myoutdir=${outroot}/5_phase,reftype=HRC -N 5_${prefix}'
job6='qsub 6_impute.job -v myinput=${outroot}/5_phase/${prefix}/${prefix}.lifted_NCBI36_to_GRCh37.GH.ancestry-${anc}.chr${chrom}.phased.vcf.gz,myoutdir=${outroot}/6_impute,reftype=HRC -N 6_${prefix}'
...
## Main Script ##
#################
job "$job0" "$run" 0 "$start_from" "$stop_after"
job "$job1" "$run" 1 "$start_from" "$stop_after"
job "$job2" "$run" 2 "$start_from" "$stop_after"
job "$job3" "$run" 3 "$start_from" "$stop_after"
